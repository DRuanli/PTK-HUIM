 ...
 *
 * UPDATED WITH PERFORMANCE FIXES:
 * - Precompute EnhancedTransaction once and reuse
 * - Parallelize EUCS building with ConcurrentHashMap
 * - Precompute suffix sums for remaining utilities
 * - Convert recursion in mineWithPrefix to iterative DFS
 * - Fixed NPE by computing itemToRank before creating EnhancedTransactions
 *
 * @author Elio
 * @version 4.3
 */
public class ver4_4 {
    ...
    /**
     * Enhanced Transaction class with efficient storage and precomputed remaining utilities
     */
    static class EnhancedTransaction {
        ...
        EnhancedTransaction(int tid, Map<Integer, Integer> itemMap,
            ...
            this.remainingUtilities = new double[size];

            int idx = 0;
            double suffixSum = 0;
            double fullRtu = 0;

            // Forward pass to fill arrays
            for (Integer item : sortedItems) {
                ...
                if (profit != null && profit > 0) {
                    fullRtu += profit * quantities[idx];
                }
                idx++;
            }

            // Backward pass for remaining utilities (excluding self)
            suffixSum = 0;
            for (int i = size - 1; i >= 0; i--) {
                remainingUtilities[i] = suffixSum;
                Double profit = profits.get(items[i]);
                if (profit != null && profit > 0) {
                    suffixSum += profit * quantities[i];
                }
            }
    ...
    public ver4_4(Map<Integer, Double> itemProfits, int k, double minPro) {
    ...
    private Map<Integer, ThreadSafeUtilityList> initialize(List<Transaction> rawDatabase) {
        ...
        // Precompute enhanced transactions once
        List<EnhancedTransaction> enhancedDatabase = rawDatabase.parallelStream()
            .map(trans -> new EnhancedTransaction(trans.tid, trans.items, trans.probabilities, itemProfits, itemToRank))
            .collect(Collectors.toList());

        // Build EUCS if appropriate
        if (itemToRank.size() <= EUCS_MAX_ITEMS) {
            buildThreadSafeEUCS(enhancedDatabase);
        }

        // Build utility lists
        return buildUtilityLists(enhancedDatabase);
    }

    private void buildThreadSafeEUCS(List<EnhancedTransaction> enhancedDatabase) {
        ConcurrentHashMap<Integer, ConcurrentHashMap<Integer, Double>> tempEucs = new ConcurrentHashMap<>();

        enhancedDatabase.parallelStream().forEach(enhanced -> {
            ...
            for (int p = 0; p < enhanced.items.length - 1; p++) {
                ...
                for (int q = p + 1; q < enhanced.items.length; q++) {
                    ...
                    tempEucs.computeIfAbsent(minItem, k -> new ConcurrentHashMap<>())
        ...
        for (Map.Entry<Integer, ConcurrentHashMap<Integer, Double>> entry : tempEucs.entrySet()) {
    ...
    private Map<Integer, ThreadSafeUtilityList> buildUtilityLists(List<EnhancedTransaction> enhancedDatabase) {
        Map<Integer, List<ThreadSafeUtilityList.Element>> tempElements = new HashMap<>();

        for (EnhancedTransaction enhanced : enhancedDatabase) {
    ...
    private void mineWithPrefix(ThreadSafeUtilityList prefix, List<ThreadSafeUtilityList> extensions) {
        Deque<AbstractMap.SimpleEntry<ThreadSafeUtilityList, Integer>> stack = new ArrayDeque<>();
        stack.push(new AbstractMap.SimpleEntry<>(prefix, 0));

        while (!stack.isEmpty()) {
            AbstractMap.SimpleEntry<ThreadSafeUtilityList, Integer> current = stack.pop();
            ThreadSafeUtilityList currPrefix = current.getKey();
            int startIdx = current.getValue();

            for (int i = startIdx; i < extensions.size(); i++) {
                ...
                ThreadSafeUtilityList joined = join(currPrefix, extension);
                ...
                // Push next level if remaining extensions
                if (i + 1 < extensions.size()) {
                    stack.push(new AbstractMap.SimpleEntry<>(joined, i + 1));
                }
            }
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        ver4_4 algorithm = new ver4_4(profits, k, minPro);
